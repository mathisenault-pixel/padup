  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-7">
          {/* Header */}
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-slate-900">Paramètres du club</h2>
          ...
          <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl text-slate-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                  />
            ...
            <div className="p-5 bg-gradient-to-br from-amber-50 to-white border border-amber-100 rounded-xl">
              <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-700 font-semibold mb-2">Début</label>
                  <input
                    type="time"
                    value={peakStart}
                ...
                <div>
                  <label className="block text-sm text-slate-700 font-semibold mb-2">Fin</label>
                  <input
                    type="time"
                    value={peakEnd}
            ...
            <div className="p-5 bg-gradient-to-br from-slate-50 to-white border border-slate-200 rounded-xl">
              ...
              <div className="max-w-sm">
                <label className="block text-sm text-slate-700 font-semibold mb-2">Minutes</label>
                <input
                  type="number"
                  min="15"
                  step="15"
            ...
            <div className="p-5 bg-white border border-slate-200 rounded-xl">
              ...
              <div className="space-y-3">
                {(Object.keys(openHours) as Array<keyof OpenHours>).map((day) => (
                  <div key={day} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <div className="font-semibold text-slate-900">{dayLabels[day]}</div>
                    <div>
                      <label className="block text-xs text-slate-600 font-semibold mb-1">Ouverture</label>
                      <input
                        type="time"
                        value={openHours[day].open}
                        onChange={(e) => updateDay(day, "open", e.target.value)}
                        className="w-full px-3 py-2 bg-white border-2 border-slate-200 rounded-lg text-slate-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                      />
                    </div>
                    <div>
                      <label className="block text-xs text-slate-600 font-semibold mb-1">Fermeture</label>
                      <input
                        type="time"
          ...
          {/* Bouton Sauvegarder */}
          <div className="mt-8 pt-6 border-t-2 border-slate-200 flex justify-end">
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="px-8 py-4 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 font-bold shadow-lg shadow-blue-200 text-base"
            >
              {isSaving ? (
                <>
          ...
          <div className="mt-6 pt-4 border-t border-slate-200">
            <p className="text-xs text-slate-500 text-center font-medium">
              ℹ️ Ces paramètres affectent le calcul des revenus et du taux d'occupation dans le dashboard.
            </p>
          </div>
        </div>
    </section>
