...
export default function PlayerAuthLayout({
  ...
  return (
    <div className={`min-h-screen w-full overflow-x-hidden ${isAccueil ? '' : 'bg-white'}`}>
      ...
      <header
        className={`fixed top-0 left-0 w-full z-50 pt-[env(safe-area-inset-top)] bg-[#F0F0F0] border-b border-slate-200 transition-all duration-300`}
      >
        ...
        {isAccueil && (
          <div className="h-[88px] flex items-center justify-center md:h-auto md:block">
      ...
      <main className={`w-full overflow-x-hidden lg:pb-0 pb-[calc(2.4cm+env(safe-area-inset-bottom,0px))] ${isAccueil ? 'pt-0' : 'pt-14 md:pt-16 min-h-screen'}`}>
        {children}
      </main>
