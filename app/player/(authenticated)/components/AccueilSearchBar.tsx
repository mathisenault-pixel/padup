  ...
  return (
    <div className={`px-3 md:px-6 lg:px-8 transition-all duration-300 ${compact ? 'py-1.5 md:py-2' : 'py-3 md:py-4'}`}>
      <div className="max-w-[778px] mx-auto">
        <div className={`flex flex-col sm:flex-row items-stretch sm:items-center bg-white rounded-[1.2rem] sm:rounded-full shadow-[0_2px_6px_rgba(0,0,0,0.08)] transition-all duration-300 ${compact ? 'min-h-[2.5rem] sm:min-h-0' : 'min-h-[3.5rem] sm:min-h-0'}`}>
          ...
          <div className={`flex-1 min-w-0 pl-[0.8cm] ${compact ? 'pr-2 py-1 sm:pr-2.5 sm:py-1.5' : 'pr-2.5 py-2 sm:pr-3.5 sm:py-2.5'}`}>
            <label className="block font-bold text-gray-600 text-[18px] mb-0.5">
              {t('accueil.searchOu')}
            </label>
            <input
              type="text"
              ...
              className="w-full text-[18px] text-gray-900 placeholder:text-gray-400 bg-transparent focus:outline-none"
            />
          </div>
