...
export default function AccueilSearchBar({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={`px-3 md:px-6 lg:px-8 transition-all duration-300 ${compact ? 'py-1.5 md:py-2' : 'py-3 md:py-4'}`}>
      <div className="w-full md:w-[19.7cm] max-w-full mx-auto">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center bg-white rounded-[1.2rem] sm:rounded-full border border-gray-300 w-full md:w-[19.7cm] min-h-[2rem] md:h-[1.5cm] md:min-h-[1.5cm] overflow-hidden transition-all duration-300">
