      ...
      <nav className="lg:hidden flex items-center gap-1">
        {navItems.filter(item => !item.desktopOnly).map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={`px-3 py-2 text-sm text-white font-light whitespace-nowrap transition-opacity hover:opacity-80 ${
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={`
                relative px-4 py-2 text-[14px] text-white font-light whitespace-nowrap tracking-wide transition-opacity hover:opacity-80
