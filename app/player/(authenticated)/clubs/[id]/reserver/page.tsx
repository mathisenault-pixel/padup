...
import { createSlotStartUTC, calculateSlotEnd, debugTimezone } from '@/lib/dateUtils'
...
/**
 * ⚠️ FONCTION SUPPRIMÉE - Utiliser calculateSlotEnd de @/lib/dateUtils
 * (Importée en haut du fichier)
 */
...
export default function ReservationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleFinalConfirmation = useCallback(async (withPremium: boolean) => {
    ...
    try {
      ...
      // Utilisation de la fonction timezone-aware de dateUtils
      // ============================================
      
      // Étape 1: Créer slot_start en UTC depuis date locale + heure locale (Paris)
      const slot_start = createSlotStartUTC(bookingDate, selectedSlot.start_time)
      
      // Étape 2: Calculer slot_end = slot_start + 90 minutes EXACT
      const slot_end = calculateSlotEnd(slot_start)
      
      console.log('[BOOKING TIMEZONE DEBUG]')
      console.log('  - Selected time (local Paris):', selectedSlot.start_time)
      console.log('  - Booking date:', bookingDate)
      console.log('  - ISO for DB (UTC):', slot_start)
      console.log('  - ISO slot_end (UTC):', slot_end)
      
      // ✅ DEBUG COMPLET TIMEZONE
      debugTimezone('slot_start (before insert)', slot_start)
      debugTimezone('slot_end (before insert)', slot_end)
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="max-w-md mx-auto p-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
            ...
            <Link href="/player/clubs" className="inline-block px-6 py-3 bg-slate-900 text-white rounded-lg hover:bg-slate-800 font-semibold transition-colors">
              ← Retour aux clubs
            </Link>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a href={`tel:${club.telephone}`} className="text-gray-900 font-semibold hover:text-slate-900">{club.telephone}</a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${club.email}`} className="text-gray-900 font-semibold hover:text-slate-900">{club.email}</a>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`flex-shrink-0 px-6 py-4 rounded-xl border-2 transition-all ${
                    isSelected
                      ...
                      : 'bg-white text-gray-900 border-gray-200 hover:border-slate-300'
                  }`}
                >
                  <div className="text-center">
              ...
              {terrains.map((terrain) => {
                ...
                return (
                  <div key={terrain.id} className="bg-white rounded-2xl border-2 border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    {/* Header du terrain */}
                    <div className="bg-gradient-to-r from-slate-50 to-gray-50 p-5 border-b-2 border-gray-200">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 bg-slate-900 rounded-xl flex items-center justify-center text-white text-xl font-black shadow-lg">
                            {terrain.id}
                          </div>
                          <div>
                          ...
                          return (
                            <button
                              ...
                              className={`p-3 rounded-xl border-2 font-bold transition-all ${
                                available && !isSubmitting
                                  ? 'bg-white text-gray-900 border-gray-200 hover:border-slate-300 hover:bg-slate-50 hover:scale-105'
                            ...
                            >
                              ...
                              {isSubmitting && available && (
                                <div className="text-xs mt-1 text-slate-700 font-semibold">...</div>
                              )}
                            </button>
                          )
                        })}
